...
'''
old sampling method, use sample_2 instead
'''
    ...
    if 'scale_free' in args.nargs or 'all' in args.nargs:
        ...
        for index, row in df.iterrows():
            ...
            for i in range(args.runs):
                ...
                '''  COMMENTED THIS OUT FOR THE RERUN
            ...
            df.to_csv(f'{dir}/scale_free_curvature.csv', index=False)
    ...
    if 'dag' in args.nargs or 'all' in args.nargs:
        ...
        for index, row in df.iterrows():
            ...
            for i in range(args.runs):
                ...
                #     plt.clf()
                print(f'Generated DAG, Size: {G.order()} p {p}')
                ##  Calculate the local curvatures
                ...
                if i == args.runs-1:
                    ...
                    df.loc[index, 'sectional']=sectional_avg
                    print("Average sectional: ", sectional_avg)
            df.to_csv(f'{dir}/dag_curvature.csv', index=False)
        ...
        for index, row in df.iterrows():
            ...
            for i in range(args.runs):
                ...
                if i == args.runs-1:
                    ...
                    df.loc[index, 'sectional']=sectional_avg
                    print("Average sectional: ", sectional_avg)
            df.to_csv(f'{dir}/random_digraph_curvature.csv', index=False)
    
    if 'dag_tree' in args.nargs or 'all' in args.nargs:
        df = get_dag_tree_dataframe()
        for index, row in df.iterrows():
            n = int(row['number of nodes'])
            r = int(row['rate'])
            e = int(row['new edges'])
            ...
            sectional_list = []
            for i in range(args.runs):
                print(f'{i}/{args.runs} completed')
                G = generate_poisson_tree(n,r)
                G = add_random_edges_dag(G, e)
                U = G.to_undirected()
                # if U.order()<=500 and i==0:
                #     plt.figure()
                #     plt.title(f'Random Tree-based Dag {G.order()} with rate {r} and {e} new edges')
                #     nx.draw(G, node_size=50)
                #     plt.savefig(f'Figures/DAG_Tree/1/Random Tree-based Dag {G.order()} with rate {r} and {e} new edges.png')
                #     plt.clf()
                print(f'Generated Random Tree-based DAG, Size: {G.order()} with rate {r} and {e} new edges')
                ##  Calculate the local curvatures
                ollivier = get_ollivier_curvature(G)
                print("ollivier: ", ollivier)
                ollivier_list.append(ollivier)
                ...
                if i == args.runs-1:
                    ...
                    df.loc[index, 'sectional']=sectional_avg
                    print("Average sectional: ", sectional_avg)
            df.to_csv(f'{dir}/dag_tree_curvature.csv', index=False)

    if 'random_tree' in args.nargs or 'all' in args.nargs:
        df = get_random_tree_dataframe()
        for index, row in df.iterrows():
            n = int(row['number of nodes'])
            e = int(row['new edges'])
            print(f'{index+1}/{len(df.index)} rows')
            ...
            for i in range(args.runs):
                print(f'{i}/{args.runs} completed')
                G = generate_random_tree(n,e,nx.DiGraph,args.seed)
                U = G.to_undirected()
                # if U.order()<=500 and i==0:
                #     plt.figure()
                #     plt.title(f'Random Tree {G.order()} with {e} new edges')
                #     nx.draw(G, node_size=50)
                #     plt.savefig(f'Figures/RANDOM_TREE/1/Random Tree {G.order()} with {e} new edges.png')
                #     plt.clf()
                print(f'Generated Random Tree {G.order()} with {e} new edges')
                ##  Calculate the local curvatures
                ollivier = get_ollivier_curvature(G)
                print("ollivier: ", ollivier)
                ollivier_list.append(ollivier)
                ...
                if i == args.runs-1:
                    ...
                    df.loc[index, 'sectional']=sectional_avg
                    print("Average sectional: ", sectional_avg)
            df.to_csv(f'{dir}/random_tree_curvature.csv', index=False)
